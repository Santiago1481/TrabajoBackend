...
namespace WebApi.Controllers
{
    ...
    public class DetalleModificadorController : ControllerBase
    {

        private readonly ISGeneric<DetalleModificador, DetalleModificadorDTO> _Servicio;
        public DetalleModificadorController(ISGeneric<DetalleModificador, DetalleModificadorDTO> servicio)
        {
            _Servicio = servicio;
        }

        // GET: api/detallemodificador
        [HttpGet]
        ...
        // POST: api/detallemodificador
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] DetalleModificadorDTO dto)
        {
        ...
        // PUT: api/detallemodificador
        [HttpPut]
        public async Task<IActionResult> Update([FromBody] DetalleModificadorDTO dto)
        {
            var dtoActualizado = await _Servicio.UpdateService(dto);
            return Ok(dtoActualizado); // Devuelve 200 OK + el objeto actualizado
        }

        // DELETE: api/detallemodificador/5
        [HttpDelete("{id}")]
